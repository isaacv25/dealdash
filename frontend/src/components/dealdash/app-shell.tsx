...
import { BarChart3, BookOpen, Calculator, DatabaseBackup, Eye, EyeOff, HandCoins, LogOut, PhoneCall, RefreshCcw } from "lucide-react";
        ...
        <aside className="glass-card rounded-[2rem] p-5 lg:sticky lg:top-6">
          ...
          <nav className="space-y-1">
            {navItems.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} className={`flex items-center gap-3 rounded-[1.15rem] px-4 py-3 text-sm transition ${active ? "bg-[linear-gradient(135deg,_rgba(21,94,239,0.18),_rgba(13,148,136,0.12))] font-semibold text-[var(--accent-strong)]" : "hover:bg-white/70"}`}>
          ...
          <div className="mt-6 space-y-2">
            <button className="ghost-button flex w-full items-center justify-center gap-2 text-sm" onClick={toggleFinancials} type="button">
