export default function DocsPage() {
  return (
    <section className="glass-card rounded-[2rem] p-6 lg:p-8 space-y-10">
      <div>
        <div className="pill bg-[rgba(21,94,239,0.12)] text-[#155eef]">Docs</div>
        <h2 className="mt-2 text-xl font-semibold tracking-tight">How DealDash works now</h2>
        <p className="mt-1 text-sm leading-6 text-[#5f7089]">This build now uses shared database persistence, account-based access, company-owned data, and financial visibility controls instead of browser-only storage.</p>
      </div>
      <DocSection title="Authentication and accounts">
        <p>DealDash now uses username and password authentication. The first account created in the system boots the legacy CSV-backed book into its company workspace and becomes that company&apos;s admin user.</p>
        <p className="mt-3">Each user belongs to exactly one company today. Future sessions can extend this into a richer membership or invite model without changing the ownership boundaries of the deal records.</p>
      </DocSection>
      <DocSection title="Data ownership and persistence">
        <p>Funded deals, pipeline records, follow-ups, and import batches are stored in Postgres via Prisma. Every row is scoped to a company ID, and every mutation runs through a server-side auth check before it can read or write that company&apos;s records.</p>
        <p className="mt-3">Browser state is now just an optimistic UI layer. The database is the system of record, so edits follow the user across devices and sessions.</p>
      </DocSection>
      <DocSection title="Dashboard reminders">
        <p>The dashboard has three quick-view rails you can clear item by item. <span className="font-semibold">Upcoming follow-ups</span> lists a follow-up about a month after you added it &mdash; hit Acknowledge once you&apos;ve handled it. <span className="font-semibold">Upcoming renewals</span> lists any funded deal that&apos;s 35% or more paid down, ready to pitch again; Dismiss removes it. <span className="font-semibold">Need new statements</span> lists pipeline leads that have rolled into a new month (last month&apos;s bank statements are stale) &mdash; hit &ldquo;Got statements&rdquo; to snooze it until next month, or mark the lead Bad Deal/Blacklisted to drop it for good. Blacklisted, declined, and funded leads never appear there.</p>
        <p className="mt-3">Your import history moved to the Imports page, so the dashboard stays focused on what needs attention today.</p>
      </DocSection>
      <DocSection title="Funded progress and commissions">
        <p>Each funded deal shows a compact summary card; click it to expand the full editor, which now includes Phone and Email right alongside name, business, and funder. A payment schedule generates automatically the moment a deal has valid terms &mdash; no need to click &ldquo;Recalculate schedule&rdquo; yourself. Once a schedule exists, the progress bar tracks the <em>actual</em> payments the system posts each day, so the board updates automatically as the deal is paid. Daily deals pay Monday&ndash;Friday (skipping the 11 US federal holidays too, not just weekends) and start the day after funding; weekly deals start a full week out on your chosen payment weekday. You can still override the balance directly whenever you know the exact live payoff.</p>
        <p className="mt-3">Every deal has a type: <span className="font-semibold">MCA</span> (default), <span className="font-semibold">HELOC</span> (prices on Amount/APR/Term-years instead of a factor rate), <span className="font-semibold">Renewal</span>, or <span className="font-semibold">Add-on</span> (both can link back to the original MCA deal so a client&apos;s history is traceable). Linking a Renewal to its original deal marks that original deal Paid in Full automatically, since a renewal pays off the old balance. Every deal also has a PSF $ fee, paid out at your broker split % and tallied into Total Payout alongside commission.</p>
        <p className="mt-3">Number fields (rate, points, payment amounts) are plain text boxes that accept decimals &mdash; type <span className="font-semibold">7.45</span> or <span className="font-semibold">10.4</span> directly, no spinner needed. Payment $ auto-fills as you edit funded amount, factor rate, term, or frequency, using the same math as &ldquo;Recalculate schedule&rdquo; &mdash; you can still type over it to match a funder&apos;s exact invoice.</p>
        <p className="mt-3">Commission payout status is tracked separately from deal performance status. That lets a file stay active while the commission itself is still pending, already paid out, or in clawback.</p>
      </DocSection>
      <DocSection title="Pipeline board">
        <p>Leads sort most-recent-first, grouped into collapsible month sections. Search matches any field on a lead &mdash; business, contact, email, phone, request, city/state, notes, lead sheet &mdash; so one search box finds a match wherever it lives.</p>
        <p className="mt-3"><span className="font-semibold">Lead sheets</span> replace the old free-text &ldquo;raw status&rdquo; field: build up a reusable list of your sources (&ldquo;Sheet A&rdquo;, &ldquo;Facebook Leads&rdquo;, ...) with the toolbar&apos;s &ldquo;+&rdquo; control, then assign each lead to one from a dropdown. The toolbar&apos;s Lead Sheet filter shows how many deals came from each sheet right in the option list, so it doubles as a source breakdown.</p>
        <p className="mt-3">Phone numbers format live as you type &mdash; anywhere a phone field appears (Pipeline, Follow-Ups, Funded deals), typing digits produces <span className="font-semibold">(555) 123-4567</span> automatically.</p>
      </DocSection>
      <DocSection title="Hidden financials">
        <p>Dashboard KPI cards each have their own eye/eye-off toggle to reveal or hide that specific number. The Rate Calculator is the one exception: its output is always visible, since it&apos;s deal-pricing math you need to see, not sensitive data pulled from your records.</p>
      </DocSection>
      <DocSection title="CSV imports and manual entry">
        <p>Upload a CSV, then pick where it goes: Funded Progress, Pipeline, or Follow-Ups. A mapping table lets you match your file&apos;s columns to DealDash&apos;s fields -- likely matches are pre-filled, but every field is a dropdown you can change, so your sheet never has to match a fixed header format.</p>
        <p className="mt-3">Once you confirm the mapping, the normalized rows are handed to a server action. The server scopes each imported row to the company and upserts by a stable row key so re-importing the same source file updates instead of duplicating.</p>
        <p className="mt-3">Manual add/edit/delete actions also persist immediately to the database through server actions. No more device-specific local snapshots.</p>
      </DocSection>
      <DocSection title="Rate calculator assumptions">
        <p>Enter funded amount, factor rate, fees, term, ISO points %, rep points %, syndication %, and a bonus. Net funded amount is funded amount minus fees; total payback is funded amount times factor rate; payment amount splits total payback evenly across the term (one payment per term-unit period -- there&apos;s no separate frequency control). Rep profit adds your cut of the ISO points, your cut of the syndicated portion, and the bonus -- it intentionally does not model syndicator management fees, so treat it as a directional number, not a fully-loaded one.</p>
      </DocSection>
      <DocSection title="Deployment requirements">
        <p>Production now requires a Postgres DATABASE_URL plus a SESSION_SECRET. Before the app can be used on a fresh environment, run Prisma against that database so the account and deal tables exist.</p>
      </DocSection>
    </section>
  );
}

function DocSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="border-t border-[rgba(19,34,56,0.1)] pt-8">
      <h3 className="text-base font-semibold tracking-tight">{title}</h3>
      <div className="mt-3 text-sm leading-7 text-[#5f7089]">{children}</div>
    </div>
  );
}
